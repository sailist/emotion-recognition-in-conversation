...
class MMINMissTrainer(Trainer, callbacks.TrainCallback, callbacks.InitialCallback):
    ...
    def imodels(self, params: ParamsType):
        ...
        self.model = MMINMissModule(visual_dim=342,
                                    text_dim=1024,
                                    audio_dim=130,
                                    n_classes=params.n_classes)

        self.pretrained_model = MMINBaseModule(visual_dim=342,
                                               text_dim=1024,
                                               audio_dim=130,
                                               n_classes=params.n_classes)
        if params.pretrain_path is not None:
            sd = torch.load(params.pretrain_path, map_location='cpu')
            self.pretrained_model.load_state_dict(sd['models']['model'])
        ...
        self.pretrained_model.apply(efficiency_init)
    ...
    def train_step(self, batch, params: ParamsType = None) -> MetricType:
        ...
        ys = batch['label']
        logits, fusion, fusion_cycle, features = self.model.forward(**batch)

        reverse_features = self.pretrained_model.encode(
        ...
        Lmse = F.mse_loss(reverse_features, fusion)
        Lcycle = F.mse_loss(features, fusion_cycle)

        Lall = Lce + Lmse * 4 + Lcycle * 2
