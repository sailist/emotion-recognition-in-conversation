...
class MaskedEdgeAttention(nn.Module):
    ...
    def forward(self, M, lengths, edge_ind):
        ...
        if attn_type == 'attn1':
            ...
            mask = torch.ones(alpha.size(), device=device) * 1e-10
            mask_copy = torch.zeros(alpha.size(), device=device)
            ...
            scores = torch.zeros(M.size(1), self.max_seq_len, self.max_seq_len, requires_grad=True, device=device)
...
def pad(tensor, length):
    if isinstance(tensor, Variable):
        ...
        if length > var.size(0):
            return torch.cat([var, torch.zeros(length - var.size(0), *var.size()[1:], device=var.device)])
        else:
            return var
    else:
        if length > tensor.size(0):
            return torch.cat([tensor, torch.zeros(length - tensor.size(0), *tensor.size()[1:], device=tensor.device)])
        else:
            return tensor
...
def batch_graphify(features, qmask, lengths, window_past, window_future, edge_type_mapping, att_model):
    """
    ...
    edge_index = torch.stack(edge_index).transpose(0, 1).to(device)
    edge_norm = torch.stack(edge_norm).to(device)
...
def attentive_node_features(emotions, seq_lengths, umask, matchatt_layer):
    ...
    input_conversation_length = torch.tensor(seq_lengths, device=device)
    start_zero = input_conversation_length.data.new(1).zero_().to(device)

    max_len = max(seq_lengths)

    start = torch.cumsum(torch.cat((start_zero, input_conversation_length[:-1])), 0)

    emotions = torch.stack([pad(emotions.narrow(0, s, l), max_len)
...
def classify_node_features(emotions, seq_lengths, umask, matchatt_layer, linear_layer, dropout_layer, smax_fc_layer,
    ...
    if nodal_attn:
        ...
        emotions = attentive_node_features(emotions, seq_lengths, umask, matchatt_layer)
...
class GraphNetwork(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_norm, edge_type, seq_lengths, umask, nodal_attn, avec):
        out = self.conv1(x, edge_index, edge_type, edge_norm=edge_norm)
        out = self.conv2(out, edge_index)
        emotions = torch.cat([x, out], dim=-1)
        logits = classify_node_features(emotions, seq_lengths, umask, self.matchatt, self.linear, self.dropout,
                                        self.smax_fc, nodal_attn, avec)
        return logits
