...
@author: lianshi.zl
@license: Ali Licence
@contact: lianshi.zl@alibaba-inc.com
@site:
@software: PyCharm Community Edition
@file: features.py
@time: 2020/4/21 7:49 下午
"""
...
def win_func_blackman(x):
    return 0.42 - 0.5 * np.cos(2.0 * np.arange(0, x, dtype=np.float32) * np.pi / x) + 0.08 * np.cos(4.0 * np.arange(0, x, dtype=np.float32) * np.pi / x)
...
def get_mel_filter(fb_dim=40, fft_size=512, sample_rate=16000, low_freq=0.0, high_freq=8000.0):
...
def wav_to_fb(sig, frm_size=400, frm_sft=160, fft_size=512, win_func=win_func_hamming, low_freq=0.0, high_freq=8000.0, fb_dim=40):
    cn, sig_len = sig.shape
    frm_num = (sig_len - frm_size) // frm_sft + 1
    ...
    # fb
    mel_filter = get_mel_filter(fb_dim=fb_dim, fft_size=fft_size, sample_rate=16000, low_freq=low_freq, high_freq=high_freq)
...
def wav_to_mfcc(sig, frm_size=400, frm_sft=160, fft_size=512, win_func=win_func_povey, low_freq=0.0, high_freq=8000.0, fb_dim=40, mfcc_dim=23):
    cn, sig_len = sig.shape
    frm_num = (sig_len - frm_size) // frm_sft + 1
